...
public class FireBrewingRecipe extends SmolderingRecipe{
    ...
    public boolean matches(SmolderingRecipeInput input, Level level) {
        if(input.fireLevel < 2 || input.getTier() < 4)
            return false;
        if(!input.getFluid().is(Fluids.WATER) || input.getFluid().getAmount() < 1000)
            return false;
        if(input.item.isEmpty())
            return false;
        ItemStack reagent = input.item.getLast();
        PotionContents potionInput = input.getFluid().getOrDefault(DataComponents.POTION_CONTENTS, new PotionContents(Potions.WATER));
    ...
    public NonNullList<ItemStack> assembleResults(SmolderingRecipeInput input, HolderLookup.Provider registries) {
        ...
        ItemStack reagent = input.item.getLast();
        ItemStack potionItem = new ItemStack(Items.POTION);
        potionItem.set(DataComponents.POTION_CONTENTS, input.getFluid().getOrDefault(DataComponents.POTION_CONTENTS, new PotionContents(Potions.WATER)));
